        ...
        dataSrc: function (response) {
            ...
            if (CheckResponseIsSuccess(response) && response.data != null) {
                ...
                if (response.data.length === 0) {
                    ...
                    $(row).appendTo('#calc_shipping_provinces');
                } else {
...
const columnTable = function () {
    return [
        ...
        {
            data: "id",
            render: (data, type, row, meta) => `<a class="table_a_href" onclick="ShowViewModal(this, '${row.id}')" href="javascript:void(0);">${row.name}</a>`,
        },
        {
            data: "status",
...
function ShowAddModal(elm) {
    ...
    $.get(`/People/P_Add`).done(function (response) {
