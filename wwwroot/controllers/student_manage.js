...
const dataScore = function (data) {
    let html = ``;
    html += `<th>${data.fifteenMinutes}</th>
<th>${data.sixtyMinutes}</th>
<th>${data.midTerm}</th>
<th>${data.lastTerm}</th>`;
    return html;
}
...
const columnTable = function () {
    return [
        ...
        {
            data: "avatar",
            render: (data, type, row, meta) => IsNullOrEmty(data) ? '' : `<img class="img img-thumbnail" src="${data}" style="width:60px;height:60px;object-fit:cover;" alt="avatar" ${_imageErrorUrl.square}' />`,
            className: "text-center text-nowrap",
        },
        {
            data: "className",
        ...
        {
            data: "maths",
            render: (data) => IsNullOrEmty(data) ? '' : dataScore(data),
            className: "text-center text-nowrap",
        }, //physics
        {
            data: "physics",
